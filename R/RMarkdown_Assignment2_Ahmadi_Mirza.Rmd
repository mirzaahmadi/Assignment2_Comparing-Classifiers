...
dfCOI_filtered <- dfCOI %>%
  ...
  filter(str_count(Sequence, "N") <= 
           (missing_data * str_count(Sequence))) %>%
  # Remove sequences that are not close to the median to homogenise data
  filter(str_count(Sequence) >= 
           median(str_count(Sequence)) - length_var 
         & str_count(Sequence) <= 
           median(str_count(Sequence)) + length_var)
...
heatmap(matrix_heatmap,
  ...
  col = colorRampPalette(c("hotpink", "mediumpurple4", "lightseagreen"))(1000),
  xlab = "Dinucleotides")
```
