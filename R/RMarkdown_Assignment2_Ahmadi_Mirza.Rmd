...
Primary author: Mirza Ali Murtuza Ahmadi

Secondary author: Phoenix Armstrong

Group members: Fatemeh Asgarian, Dilma Karunathilake
...
dfCOI <- data.frame(Title = names(coi_stringSet), 
                    Sequence = paste(coi_stringSet))
dfCytB <- data.frame(Title = names(cytb_stringSet), 
                     Sequence = paste(cytb_stringSet))
```
...
explore_data <- function(df) {
  ...
  cat("Total number of NA values in sequence column:", sum(is.na(df[,2])), "\n")
  cat("Total number of gaps in sequence columns:", sum(str_count(df[,2], "-")), "\n")

  # Summary of sequence data
  cat("Summary of sequence data:", "\n")
  print(summary(str_count(df[,2]))) # This prints the typical summary stats

  total_A <- sum(str_count(df[,2], "A"))
  total_T <- sum(str_count(df[,2], "T"))
  total_C <- sum(str_count(df[,2], "C"))
  total_G <- sum(str_count(df[,2], "G"))
  total_N <- sum(str_count(df[,2], "N"))
...
# create data filtering function to expand on usage of explore_data() function
filter_seq <- function(df, range = 0.9, N_thresh = 0.01, len = 100, by = NULL) {
  if (is.null(by)) {
    by <- "range"
  } # default
  alpha <- (1 - range)
  q <- quantile(str_length(df$Sequence), probs = c(alpha / 2, 0.5, 1 - alpha / 2))
  filtered <- df %>%
    # remove Ns at start or end of sequence to homogenise data
    mutate(Sequence = str_remove_all(Sequence, "^N+|N+$|-")) %>%
    # To ensure high quality data, remove any sequences with over 1% of ambiguous nucleotides
    filter(str_count(Sequence, "N") <=
      (N_thresh * str_length(Sequence)))
  # Remove sequences that are not close to the median to homogenise data
  if (by == "range") {
    filtered <- filtered %>%
      filter(str_length(Sequence) >= q[1] &
        str_count(Sequence) <= q[3])
  } else if (by == "length") {
    filtered <- filtered %>%
      filter(str_length(Sequence) >= q[2] - len &
        str_length(Sequence) <= q[2] + len)
  } else {
    stop("Please filter by range or by length.")
  }

  explore_data(filtered)
  return(filtered)
}

# Create new filtered COI and CytB datasets
dfCOI_filtered <- filter_seq(dfCOI, by = "length")
dfCytB_filtered <- filter_seq(dfCytB, by = "length")
```

## 4. Sequence Length Visualization and K-Mer Frequency Visualization

```{r, fig.cap = "Histogram showcases the distribution of filtered sequence lengths for COI and CytB. This shows the variation between sequence lengths in COI and CytB, in order to get a better grasp of the lengths of sequence data we are working with as well as showcasing a visual distinction between the sequence lengths of the different genes. The figure shows two normal distributions around the median sequence length around each gene type, with them being distant from each other on the x axis, indicating the differences in sequence length between the two genes."}

seq_length_COI_filtered <- nchar(dfCOI_filtered$Sequence)
seq_length_CytB_filtered <- nchar(dfCytB_filtered$Sequence)
...
hist(seq_length_COI_filtered,
  ...
  col = rgb(1, 0, 0, 0.5),
  breaks = "Sturges",
  xlim = c(400, 1400), # Extend x-axis to accommodate sequence length of both COI and CytB
  ylim = c(0, 2000)) # Extend y-axis to accommodate sequence length of CytB
...
```{r, fig.cap = "Heatmap shows dinucleotide frequency distribution comparing COI and CytB averages. The result of this heatmap indicated the differences in dinucelotide proportions between COI and CytB in terms of dinucleotide frequency, which is an important metric that can be used whne training classifiers downstream."}

dfCOI_filtered$Sequence <- DNAStringSet(dfCOI_filtered$Sequence)
dfCytB_filtered$Sequence <- DNAStringSet(dfCytB_filtered$Sequence)

# Calculate dinucleotide frequency for each sequence for CytB and COI
dfCOI_filtered <- cbind(dfCOI_filtered, 
                        as.data.frame(dinucleotideFrequency(dfCOI_filtered$Sequence,
                                                            as.prob = TRUE)))

dfCytB_filtered <- cbind(dfCytB_filtered, 
                         as.data.frame(dinucleotideFrequency(dfCytB_filtered$Sequence,
                                                             as.prob = TRUE)))

# Calculate the averages for each dinucleotide for COI and CytB gene
average_dinucleotide_proportions_COI <- colMeans(dfCOI_filtered[, -(1:2)],
                                                 na.rm = TRUE)
average_dinucleotide_proportions_CytB <- colMeans(dfCytB_filtered[, -(1:2)],
                                                  na.rm = TRUE)

# Create the heatmap to visualize the differences in dinucleotide proportions between COI and CytB
matrix_heatmap <- t(as.matrix(data.frame(COI = average_dinucleotide_proportions_COI,
                                         CytB = average_dinucleotide_proportions_CytB),
                              byrow = TRUE))

heatmap(matrix_heatmap,
        Rowv = NA, Colv = NA, scale = "none",
        col = colorRampPalette(c("deeppink", "mediumpurple4", "lightseagreen"))(1000),
        xlab = "Dinucleotides")
```
...
dfCOI_filtered$Gene <- "COI"
dfCytB_filtered$Gene <- "CytB"
df_combined <- rbind(dfCOI_filtered, dfCytB_filtered)
...
set.seed(727)
combined_validation_df <- df_combined %>%
  group_by(Gene) %>%
  sample_n(250)
...
set.seed(277)
combined_training_df <- df_combined %>%
  group_by(Gene) %>%
  filter(!Title %in% combined_validation_df$Title) %>%
  sample_n(950)
...
predictValidation_rf <- predict(rf_gene_classifier, combined_validation_df[, -(1:2)])
...
predictValidation_lg <- predict(lr_gene_classifier, combined_validation_df[, -(1:2)])
table(observed = combined_validation_df$Gene, predicted = predictValidation_rf)
```
...
```{r, fig.cap = "Lollipop plots which shows the feature importance of different dinucleotides for the random forest and logistic regression algorithm. The different algorithms assign different importance scores to different dinucleotides due to how the different algorithms learn from the data and what they prioritize."}
importance_plot <- function(classifier, type) {
  feature_importance <- varImp(classifier)

  # Convert to data.table for easier manipulation
  imp_vec <- rowMeans(feature_importance$importance)
  imp <- data.table(
  ...
  plot <- ggplot(data = imp, aes(
    ...
    coord_flip() +
    theme_minimal() +
    labs(x = "Features", y = "Feature Importance Score")

  return(plot)
}

plot_RF <- importance_plot(rf_gene_classifier, "Random Forest")
plot_LR <- importance_plot(lr_gene_classifier, "Logistic Regression")

plot_RF + 
  scale_color_manual(name = "Classifiers: ",
                     limits = "Random Forest", 
                     values = "lightseagreen") +
  plot_LR + 
  scale_color_manual(name = "",
                     limits = "Logistic Regression", 
                     values = "hotpink") +
...
```{r, message = FALSE, warning = FALSE, fig.cap = "K-mer number vs time elapsed (A) and k-mer number vs model accuracy (B) fitted line graphs which show the speed of training and accuracy at different k mer sizes for the logistic regression and random forest algorithms. The random forest algorithm was slower at all k mer sizes, especially as k mer continued to increase the difference between the two algorithms in terms of speed grew farther and farther apart. Random forest seems to be increasingly slower with higher k mer, potentially due to the greater increase of decision tree complexity and number as k mer size continues to get larger."}
...
common_layers <- list(geom_point(size = 3),
                      scale_color_manual(name = "Classifiers: ",
                                         limits = c("Random Forest", "Logistic Regression"),
                                         values = c("lightseagreen", "hotpink")),
                      scale_shape_manual(name = "Classifiers: ",
                                         limits = c("Random Forest", "Logistic Regression"),
                                         values = c(8, 9)),
                      theme_minimal())

speed <- ggplot(combined_results, 
                aes(x = k, y = time.taken.seconds, color = model, shape = model)) +
  geom_smooth(method = "loess", 
              formula = (y ~ exp(x)), 
              span = 2, se = FALSE, size = 1) +
  common_layers +
  labs(x = "K-mer Size",
       y = "Time Taken (seconds)")

accuracy <- ggplot(combined_results, 
                   aes(x = k, y = round((accuracy*100), 2), color = model, shape = model)) +
